...
namespace bounce_back
{
    ...
    )
    {
        for(auto bsi_iterator = bsi.begin(); bsi_iterator < bsi.end(); ++bsi_iterator)
        {
            ...
            {
                distribution_values[
                    lbm_accessor.get_index(lbm_access::get_neighbor((*bsi_iterator)[0] + read_offset, *direction_iterator), invert_direction(*direction_iterator))] = 
                    distribution_values[lbm_accessor.get_index((*bsi_iterator)[0] + read_offset, *direction_iterator)];
            }
        }
    }
...
namespace boundary_conditions
{
    ...
    )
    {
        ...
        for(auto y = 1; y < VERTICAL_NODES - 1; ++y)
        {
            ...
            v = macroscopic::flow_velocity(lbm_access::get_distribution_values_of(distribution_values, lbm_access::get_neighbor(current_border_node, 3), lbm_accessor));
