        ...
        )
        {
            ...
            for(unsigned int y = simulation.domain->vertical_nodes; y-- > 0; )
            {
                for(unsigned int j = 0; j < 3; ++j)
                {
                    ...
                    {
                        current_node_index = 
                            core::access::get_node_index(x, y, simulation.domain->horizontal_nodes);

                        if(x == 0 && y == 0) 
                            std::cout << "\033[31m"; // Color origin values red

                        else if(
                            x == (simulation.domain->horizontal_nodes - 1) && 
                            y == (simulation.domain->vertical_nodes - 1)
                        ) 
                        ...
                        for(auto direction = 0; direction < 9; ++direction)
                        {
                            current_values[direction] = 
                                distribution_values[
                                    A::at(current_node_index, direction, simulation.domain->total_node_count)
                                ];
                        }
