...
{
    inline static unsigned int evaluate(const unsigned int x,
...
struct WallStencil
{
    inline static unsigned int evaluate(const unsigned int x,
                                        ...
                                        const unsigned int horizontal_nodes_org)
    {
        unsigned int cond = (x == (1 * horizontal_nodes_org) / 6 || x == (3 * horizontal_nodes_org) / 6
                             || x == (5 * horizontal_nodes_org) / 6)
                            && (y > 1) && (y < vertical_nodes_org / 2);

        cond += (x == (2 * horizontal_nodes_org) / 6 || x == (4 * horizontal_nodes_org) / 6)
...
struct CircleStencil
{
                                        ...
                                        const unsigned int horizontal_nodes_org)
    {
        ...
        unsigned int cond =
            (y >= middle_y - radius) && (y <= middle_y + radius) && (x >= middle_x - radius) && (x <= middle_x + radius)
            && (sycl::abs(middle_x - x) * sycl::abs(middle_x - x) + sycl::abs(middle_y - y) * sycl::abs(middle_y - y)
                <= radius * radius);
        return cond;
    }
};
...
struct SquareStencil
{
                                        ...
                                        const unsigned int horizontal_nodes_org)
    {
        ...
        unsigned int cond = (y >= middle_y - half_length) && (y <= middle_y + half_length)
                            && (x >= middle_x - half_length) && (x <= middle_x + half_length);
        return cond;
    }
};
...
struct PlateStencil
{
                                        ...
                                        const unsigned int horizontal_nodes_org)
    {
        ...
        unsigned int cond = (y >= middle_y - half_height) && (y <= middle_y + half_height)
                            && (x >= middle_x - half_width) && (x <= middle_x + half_width);
        return cond;
    }
};
...
struct SkyscraperStencil
{
                                        ...
                                        const unsigned int horizontal_nodes_org)
    {
        ...
        unsigned int cond =
            (y >= 1) && (y <= height_first && x >= middle_x - half_width_first && x <= middle_x + half_width_first)
            || (y <= height_second && x >= middle_x - half_width_second && x <= middle_x + half_width_second)
            || (y <= height_antenna && x >= middle_x - half_width_antenna && x <= middle_x + half_width_antenna);
        return cond;
    }
};
...
struct WingStencil
{
                                        ...
                                        const unsigned int horizontal_nodes_org)
    {
        ...
        unsigned int cond =
            ((0.2 * sycl::abs((x - middle_x) * (x - middle_x)) + sycl::abs((y - middle_y) * (y - middle_y)))
             <= radius_max)
...
template <class T>
concept GPUStencil =
    std::same_as<T, HagenPoiseuilleStencil> || std::same_as<T, WallStencil> || std::same_as<T, CircleStencil>
    || std::same_as<T, SquareStencil> || std::same_as<T, PlateStencil> || std::same_as<T, SkyscraperStencil>
...
void set_pipe_boundaries(Simulation &simulation, sycl::queue &queue)
{
    auto event = queue.submit(
        [&](sycl::handler &cgh)
        {
            unsigned int extended_horizontal_nodes = simulation.domain->horizontal_nodes;
            ...
            cgh.parallel_for(sycl::range<2>(2, simulation.properties->horizontal_nodes - 2),
                             [=](const sycl::id<2> &id)
                             {
                                 phase_information[N::get_index(id.get(1) + 1,
                                                                id.get(0) * vertical_nodes_org + 1 - 3 * id.get(0),
                                                                subdomain_vertical_nodes,
                                                                subdomain_horizontal_nodes,
                                                                extended_horizontal_nodes)] = 1;
                             });
        });
    event.wait();
}
 ...
 */
template <core::access::decomposed::NodeAccessor N>
void porous_medium_mask(const unsigned int vertical_nodes_org,
                        const unsigned int horizontal_nodes_org,
                        ...
                        std::vector<int8_t> &phase_information_cpu)
{
    for (int y = 2; y < vertical_nodes_org - 2; ++y)
    {
        for (int x = 1; x <= horizontal_nodes_org - 2; ++x)
        {
            ...
            phase_information_cpu[N::get_index(
                x, y, subdomain_vertical_nodes, subdomain_horizontal_nodes, extended_horizontal_nodes)] = value;
        }
    }
    ...
    for (int y = 1; y < vertical_nodes_org; y += vertical_nodes_org - 3)
    {
        ...
        {
            phase_information_cpu[N::get_index(
                x, y, subdomain_vertical_nodes, subdomain_horizontal_nodes, extended_horizontal_nodes)] = 1;
        }
    }
}
...
void set_standstill_values(Simulation &simulation, sycl::queue &queue)
{
    real_type values[9];
    ...
    auto event = queue.submit(
        [&](sycl::handler &cgh)
        {
            cgh.parallel_for(total_node_count,
                             [=](const sycl::id<1> &idx)
                             {
...
void set_inout_distribution_values(Simulation &simulation, sycl::queue &queue)
{
    real_type values[18];
    ...
    int8_t buffered = ((vertical_nodes - subdomain_vertical_nodes * vertical_subdomain_count) > 2)
                      || ((horizontal_nodes - subdomain_horizontal_nodes * horizontal_subdomain_count) > 2);

    auto event = queue.submit(
        [&](sycl::handler &cgh)
        {
            cgh.parallel_for(sycl::range<2>(2, simulation.properties->vertical_nodes - 2),
                             [=](const sycl::id<2> &id)
                             {
        ...
        [&](sycl::handler &cgh)
        {
            ...
            cgh.parallel_for(
                ...
                [=](const sycl::id<2> &id)
                {
                    ...
                    phase_information[N::get_index(
                        x, y, subdomain_vertical_nodes, subdomain_horizontal_nodes, extended_horizontal_nodes)] =
...
void setup_domain(Simulation &simulation, sycl::queue &queue)
{
    ...
    else if (simulation.properties->scenario == "porous")
    {
        ...
        queue
            .copy(phase_information_cpu.data(), simulation.data->phase_information, simulation.domain->total_node_count)
            .wait();
    }
}
