 ...
 *              kernel functions are implemented in `lptl_kernels.hpp`.
 *
...
/**
 * @brief This namespace contains the LPTL (Linear Pull Two-Lattice) algorithm.
 */
namespace lptl
{
...
class LPTL : public execution::SYCLAlgorithm
{
  private:
    /**
    ...
    inline void stream_and_collide()
    {
        auto event = queue->submit(
            [&](sycl::handler &cgh)
            {
                auto kernel = lptl::kernels::StreamCollideKernel<A>(*simulation);
                cgh.parallel_for(
  ...
  public:
    ...
    /**
     * @brief   Constructs a `LPTL` algorithm object and initializes its domain.
     *
     * @param[in]   queue           the SYCL queue used for interaction with the device
     * @param[in]   settings_path   path to the JSON settings file
     */
    explicit LPTL(sycl::queue &queue, const std::string &settings_path) :
    ...
    explicit LPTL(sycl::queue &queue, const core::Properties &props) :
 ...
 */
template <core::access::AccessorConcept A>
class LPTLDebug : public execution::SYCLAlgorithm
{
  private:
    ...
    void stream()
    {
        queue
            ->submit(
                [&](sycl::handler &cgh)
                {
                    auto kernel = lptl::kernels::StreamKernel<A>(*simulation);
    ...
    void update_macroscopic_observables()
    {
        queue
            ->submit(
                [&](sycl::handler &cgh)
                {
                    auto kernel = lptl::kernels::MacroscopicObservablesKernel<A>(*simulation);
                    cgh.parallel_for(sycl::range<1>(simulation->properties->total_unexpanded_node_count), kernel);
                })
            .wait();
    }
    ...
    void collide()
    {
        queue
            ->submit(
                [&](sycl::handler &cgh)
                {
                    auto kernel = lptl::kernels::CollideKernel<A>(*simulation);
                    cgh.parallel_for(sycl::range<1>(simulation->properties->total_unexpanded_node_count), kernel);
                })
            .wait();
    }
  ...
  public:
    ...
    /**
     * @brief   Constructs a new `LPTLDebug` algorithm object and initializes its domain.
     *
     * @param[in]   queue           the SYCL queue used to allocate the device data
     * @param[in]   settings_path   path to the JSON settings file
     */
    explicit LPTLDebug(sycl::queue &queue, const std::string &settings_path) :
        SYCLAlgorithm(queue, settings_path),
    ...
    explicit LPTLDebug(sycl::queue &queue, const core::Properties &props) :
        SYCLAlgorithm(queue, props),
        all_densities(std::make_unique<std::vector<real_type>>()),
        all_x_velocities(std::make_unique<std::vector<real_type>>()),
        all_y_velocities(std::make_unique<std::vector<real_type>>()),
