                    ...
                    class MacroscopicObservablesKernel
                    {
                        ...
                        void operator()(const sycl::item<1> &item) const
                        {
                            ...
                            if(!core::is_ghost_node(id, vertical_nodes, horizontal_nodes, phase_info_accessor[id]))
                            {
                                ...
                                for (const auto& direction : core::constants::all_directions)
                                {
                                    dist_vals[direction] = destination_accessor[A::at(id, direction, horizontal_nodes * vertical_nodes)];
                    ...
                    namespace boundaries
                    {
                        ...
                        class InoutUpdateKernel
                        {
                            ...
                            void operator()(const sycl::item<1> &item) const
                            {
                                ...
                                unsigned int current_border_node = core::access::get_node_index(horizontal_nodes - 2, id, horizontal_nodes);
                                ...
                                for(int i = 0; i < 3; ++i)
                                {
                                    f_inverse[i] = distribution_values_accessor[
                                        A::at(
                                            core::access::get_neighbor(current_border_node, missing[i], horizontal_nodes), 8 - missing[i], horizontal_nodes * vertical_nodes
                                        )];
                                }

                                double x_velocity = 1 - (1 / outlet_density) * (f_1 + f_4 + f_7 + 2 * (f_inverse[0] + f_inverse[1] + f_inverse[2]));
                        ...
                        )
                        {
                            ...
                            //     // Setup actual coordinates
                            //     id_y = id_y + 2;                                         // y coordinate: offset by two
                            //     id_x = properties.horizontal_nodes - 2;                // x coordinate: 0 -> 1, 1 -> horizontal_nodes - 2
