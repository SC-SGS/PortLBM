        ...
        if (!phase_information[linear_index])
        {
            for (int direction = 0; direction < 9; ++direction)
            {
                destination[A::at(linear_index, direction, horizontal_nodes * vertical_nodes)] =
                    source[A::at(lbm::core::access::get_neighbor(linear_index, 8 - direction, horizontal_nodes),
                                 direction,
    ...
    void operator()(const sycl::nd_item<2> &nd_item) const
    {
        ...
        if (!phase_information[linear_index])
        {
            ...
            for (int direction = 0; direction < 9; ++direction)
            {
                ...
                destination[A::at(linear_index, direction, horizontal_nodes_expanded * vertical_nodes_expanded)] =
                    result;
            }
