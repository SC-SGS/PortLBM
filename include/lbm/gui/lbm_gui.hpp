...
class Gui
{
  private:
    ...
    inline void run_button()
    {
        ...
        if (ImGui::Button("run", ImVec2(1.0 / 4 * ImGui::GetWindowSize().x * 0.5f, 0.0f)))
        {
            ...
            if (!simulation_control->is_simulation_active)
            {
                ...
                properties_gui =
                    std::make_unique<core::Properties>(file_interaction::json_to_properties(settings_path_, -2));
    ...
    inline void abort_button()
    {
        ...
        if (ImGui::Button("abort", ImVec2(1.0 / 4 * ImGui::GetWindowSize().x * 0.5f, 0.0f))
    ...
    inline void properties_simulation_and_domain()
    {
        ...
        if (ImGui::InputUnsignedInt("Time steps", &(properties_gui->time_steps), 1, 10)
    ...
    {
        if (ImGui::InputUnsignedInt("Work group size", &(properties_gui->work_group_size), 1, 10))
        {
            properties_changed = true;
    ...
    inline void simulation_status_window()
    {
        ...
        {
            ImGui::SetNextWindowSize({ 1 * monitor->viewport->WorkSize.x / 4, 3 * monitor->viewport->WorkSize.y / 20 });
    ...
    inline void framerate_window()
    {
        ...
        {
            ImGui::SetNextWindowSize({ 1 * monitor->viewport->WorkSize.x / 4, 7 * monitor->viewport->WorkSize.y / 20 });

            ImGui::SetNextWindowPos({ 0, windows->menu_bar_size + 3 * monitor->viewport->WorkSize.y / 20 });

            if (ImGui::Begin("Framerate", &windows->show_simulation_status, ImGuiWindowFlags_NoResize))
            {
                ImGui::Text(
                    "Backend: %.1f FPS, %.3f ms/frame ", progress->framerate_backend, progress->frametime_backend);

                ImGui::Text(
                    "Frontend: %.1f FPS, %.3f ms/frame ", progress->framerate_frontend, progress->frametime_frontend);
                                      ...
                                      ImPlotFlags_NoLegend | ImPlotFlags_NoMouseText))
                {
                    ...
                    ImPlot::SetupAxisLimits(
                        ImAxis_X1, fps_buffer->current_time - 10, fps_buffer->current_time, ImGuiCond_Always);
    ...
    inline void properties_window()
    {
        ...
        {
            ImGui::SetNextWindowSize({ 1 * monitor->viewport->WorkSize.x / 4, 5 * monitor->viewport->WorkSize.y / 10 });

            ImGui::SetNextWindowPos({ 0, windows->menu_bar_size + 5 * monitor->viewport->WorkSize.y / 10 });
            ...
            if (ImGui::Begin("Properties", &windows->show_properties, ImGuiWindowFlags_NoResize))
            {
                ...
                scenario_selection();
                if (ImGui::InputUnsignedInt("Frame update interval", &(properties_gui->frame_update_interval), 1, 10))
                {
                    properties_changed = true;
                }
    ...
    void debug_message()
    {
        ...
        {
            ImGui::SetNextWindowSize({ monitor->viewport->WorkSize.x / 3, 0 });

            ImGui::SetNextWindowPos(
                { monitor->viewport->WorkSize.x / 3, windows->menu_bar_size + monitor->viewport->WorkSize.y / 3 });

            if (ImGui::Begin("Debug message",
                             &windows->show_debug_window,
                             ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
            {
                ImGui::TextColored({ 255, 0, 0, 1 }, "Debug mode detected.");
                ImGui::TextWrapped(
                    "In debug mode, the absolutes of all velocity values and the density values are printed on the "
                    "node. "
    ...
    inline void calculate_velocity_quiver()
    {
        if (windows->enable_velocity_quiver)
        {
            ...
            velocity_quiver_data->x_values->assign(
                2 * algorithm_handler->get_horizontal_nodes() * algorithm_handler->get_vertical_nodes(), 0);
            ...
            for (int y = 1; y < algorithm_handler->get_vertical_nodes() - 1; ++y)
            {
                ...
                {
                    inner_node_index =
                        core::access::get_node_index(x - 1, y - 1, algorithm_handler->get_horizontal_nodes() - 2);

                    node_index = core::access::get_node_index(x, y, algorithm_handler->get_horizontal_nodes());

                    velocity_value_index = core::access::get_result_index(
                    ...
                    if (algorithm_handler->get_absolute_velocities().at(velocity_value_index) > 1e-15)
                    {
                        ...
                        offset_x = base_x
                                   + 0.5 * (1.0 / algorithm_handler->get_absolute_velocities().at(velocity_value_index))
    ...
    void density_window()
    {
        ...
        {
            ImGui::SetNextWindowPos(ImVec2((1.0 / 4) * monitor->viewport->WorkSize.x,
                                           windows->menu_bar_size + 1.0 / 2 * monitor->viewport->WorkSize.y),
                                    ImGuiCond_Appearing);

            ImGui::SetNextWindowSize(
                ImVec2((3.0 / 4) * monitor->viewport->WorkSize.x, 1.0 / 2 * monitor->viewport->WorkSize.y),
                ImGuiCond_Appearing);
            ...
            if (ImGui::Begin("Density", &windows->show_density))
            {
                        ...
                        ImPlotFlags_NoLegend | ImPlotFlags_NoMouseText | ImPlotFlags_Crosshairs | ImPlotFlags_NoTitle))
                {
                    ...
                    ImPlot::PlotHeatmap(
                        ...
                        ImPlotPoint(0, 0),
                        ImPlotPoint(algorithm_handler->get_horizontal_nodes() - 2,
                                    algorithm_handler->get_vertical_nodes() - 2));
                    ...
                    if (ImPlot::IsPlotHovered())
                    {
                        ...
                        if (0 <= mouse.x && mouse.x < (algorithm_handler->get_horizontal_nodes() - 2) && 0 <= mouse.y
                            && mouse.y < (algorithm_handler->get_vertical_nodes() - 2))
                        {
                            ImGui::BeginTooltip();
                            ImGui::Text("Coordinates: %.2f, %.2f", mouse.x, mouse.y);
                            real_type value = algorithm_handler->get_densities().at(
                                (algorithm_handler->get_horizontal_nodes() - 2) * ((int) floor(mouse.y))
                                + (int) floor(mouse.x));
                ...
                ImPlot::ColormapScale(
                    ...
                    colormaps->density_colormap_lower_scale,
                    colormaps->density_colormap_upper_scale,
                    ImVec2(monitor->monitor_x_scale * 100,
                           ImGui::GetContentRegionAvail().y - ImGui::GetStyle().ItemSpacing.y),
                    "%g",
    ...
    void velocity_window()
    {
        ...
        {
            ImGui::SetNextWindowPos(
                ImVec2((1.0 / 4) * monitor->viewport->WorkSize.x, windows->menu_bar_size), ImGuiCond_Appearing);
                        ...
                        ImPlotFlags_NoLegend | ImPlotFlags_NoMouseText | ImPlotFlags_Crosshairs | ImPlotFlags_NoTitle))
                {
                    ...
                    if (ImPlot::IsPlotHovered())
                    {
                            ...
                            && mouse.y < (algorithm_handler->get_vertical_nodes() - 2))
                        {
                            ...
                            if (algorithm_handler->get_densities().at(
                                    (algorithm_handler->get_horizontal_nodes() - 2) * (int) floor(mouse.y)
                                    + (int) floor(mouse.x))
                                != -1)
                            {
                                ImGui::Text("x velocity: %.6f",
                                            algorithm_handler->get_x_velocities().at(
                                                (algorithm_handler->get_horizontal_nodes() - 2) * ((int) floor(mouse.y))
                                                + (int) floor(mouse.x)));
                                ImGui::Text("y velocity: %.6f",
    ...
    inline void export_benchmark_data()
    {
        ...
        double frontend_average =
            std::accumulate(benchmark_values->frontend_fps->begin(), benchmark_values->frontend_fps->end(), 0.0)
            / benchmark_values->frontend_fps->size();

        double backend_average =
            std::accumulate(benchmark_values->backend_fps->begin(), benchmark_values->backend_fps->end(), 0.0)
        ...
        std::string ofdir =
            "../benchmarks/gui/" + std::to_string(properties_gui->horizontal_nodes) + "x"
            + std::to_string(properties_gui->vertical_nodes) + "_ft_"
            + std::to_string(properties_gui->frame_update_interval) + "_"
            + std::to_string(benchmark_values->benchmark_counter) + ".json";
  ...
  public:
    explicit Gui(const std::string &&window_title, const std::string &settings_path) :
        ...
        properties_gui(
            std::make_unique<core::Properties>(lbm::file_interaction::json_to_properties(settings_path_, -2))),
        ...
        algorithm_handler(std::make_unique<A>(settings_path_))
    {
        ...
        glfw_window = glfwCreateWindow(
            monitor->video_mode->width, monitor->video_mode->height, window_title.c_str(), nullptr, nullptr);

        if (glfw_window == nullptr)
        {
            throw exceptions::Exception("Failed to create GLFW window.");
        }
