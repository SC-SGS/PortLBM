...
lbm::benchmark::Benchmark::Benchmark(std::shared_ptr<execution::SYCLAlgorithmHandler> &sycl_algorithm_handler) :
    ...
    work_group_sizes(std::make_unique<std::vector<size_t>>(get_work_group_sizes(
    ...
    optimal_total_times(std::make_unique<std::map<std::string, double>>())
{
    ...
    data_layouts = std::make_unique<std::vector<std::string>>(
        benchmark_settings.at("dataLayouts").get<std::vector<std::string>>());

    test_runs = benchmark_settings.at("testRunsPerConfig").get<unsigned int>();

    phase_2_scenarios = std::make_unique<std::vector<std::string>>(
    ...
    Progress &progress)
{
    ...
    for (const auto &data_layout : *data_layouts)
    {
        ...
        for (int test_run = 0; test_run < test_runs; ++test_run)
        {
            ...
            std::cout << "Progress: " << progress.current_test_rows << " / " << progress.total_test_rows << " ("
...
void lbm::benchmark::Benchmark::phase_2_inner_loop(
    const std::string &ofdir, nlohmann::json &file_data, Progress &progress)
{
    double runtime = 0;
    ...
    for (int test_run = 0; test_run < test_runs; ++test_run)
    {
        ...
        file_data["initializationTimes"].push_back(sycl_algorithm_handler->initialization_time);
